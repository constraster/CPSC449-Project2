...
def make_post():
    ...
    if makep:
        ...
        createtime = get_time()
        changetime = get_time()
        post = Post(username=username, title=title, text=text, subreddit=subreddit, upvotes=0, downvotes=0,
                    createtime=createtime, changetime=changetime)
        db.session.add(post)

        #create vote associated with post
        vote = Vote(upvotes=1, downvotes=0)
        db.session.add(vote)
